...
MAX_ROCKET_TIME = float(config['times']['max_rocket'])
BUZZER_WAIT = float(config['times']['buzzer_wait'])
BUZZER_TIME = float(config['times']['buzzer_time'])
ROCKET_STOP_TIME = float(config['times']['rocket_stop'])
...
def ir_detected():
    print("IR detected!")
    play_speaker(freq=BUZZER_FREQ, duration=2)
    enable_hovercraft()
...
def check_light():
    ...
    time.sleep(0.5)
    while True:
        if is_light_detected() or time.time() - start > MAX_ROCKET_TIME:
            same += 1
        else:
            same = 0
...
def start_rocket():
    print(f"Starting rocket...")
    GPIO.output(LED_PIN, GPIO.HIGH)
    GPIO.output(ROCKET_PIN, GPIO.HIGH)
 
def stop_rocket():
    time.sleep(ROCKET_STOP_TIME)
